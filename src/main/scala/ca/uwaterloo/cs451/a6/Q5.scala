  ...
  def main(argv: Array[String]) {
    ...
    if (args.text()) {
      ...
      val orders = ordersRDD
        ...
        .filter(pair => {
          val nationkey = customerMap.value(pair._2).toInt
          nationkey.equals(canada) || nationkey.equals(usa)
        })
        .map(pair => (pair._1, customerMap.value(pair._2)))
      ...
      val lineitem = lineitemRDD
        .map(line => {
          ...
          (
            tokens(0), // l_orderkey
            tokens(10).substring(0, 7) // l_shipdate
          )
        })

      lineitem.cogroup(orders)
        .filter(pair => pair._2._2.iterator.hasNext)
        .flatMap { case (_, (dates, nationkeys)) =>
          dates.map(date => {
            val nationkey = nationkeys.iterator.next()
            val name = nationMap.value(nationkey)
            ((nationkey.toInt, name, date), 1)
          })
        }
        .reduceByKey(_ + _)
        .sortByKey()
      ...
      val lineitem = lineitemRDD
        .map(row => {
          (
            ...
            row(10).toString.substring(0, 7) // l_shipdate
          )
        })
